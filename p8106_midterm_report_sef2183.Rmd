...
The final model was selected by comparing the median training RMSE for all models (Figure 2 in the appendix). The lasso model had the highest mean and median training RMSE (i.e., worst performance), and the MARS model had the lowest mean and median training RMSE (i.e., best performance). However, the MARS model only contained 4 terms with 2 of the predictors, so it was not an ideal model for comprehensively identifying important risk factors for recovery time, as so few risk factors were included. Therefore, the GAM model will all predictors--the second best model in terms of mean and median RMSE--was selected as the final model in this study.
...
**Model performance:** All of these factors together explain 43.8% of the deviance in COVID-19 recovery time. Additionally, the model's training error of 24.4 indicates that, on average, the model's predictions on the training data deviate from the actual values by 24.4 units. Meanwhile, the test error of 23.8 (see appendix) suggests that the model's performance on unseen data is slightly better than its performance on the training data. This indicates that the model generalizes well to new data. 
