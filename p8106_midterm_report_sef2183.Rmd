...
**Model interpretation:** Significant predictors of COVID-19 recovery time at the 5% level of significance include gender, smoking history, hypertension, vaccination status, infection severity, and study group. The GAM model also identified significant nonlinear relationships between age, BMI, weight, and height and predicted recovery time. On average: 
...
**Model performance:** All of these factors together explain 44.2% of the deviance in COVID-19 recovery time. Additionally, themodel’s training error (RMSE using the training dataset) of about 24.0 (rounded) indicates that, on average,the model’s predictions on the training data deviate from the actual values by 24 units. Meanwhile, thetest error (RMSE using the test dataset) of 25.5 (rounded) suggests that the model’s performance on unseendata is slightly worse than its performance on the training data.18
